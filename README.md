...
- **SQL Server čitalac** — preporuka: kreirati zaseban login `bridge_reader` sa SELECT pravima na: `tRadneJedinice`, `tOperacije`, `Komitenti`, `tRadnici`. Faza 1C dodaje još tabela.
...
-- 6) RLS — Bridge koristi SERVICE_ROLE key (bypass RLS).
...
# 3) ručno pokrenuti sva 4 odjednom
